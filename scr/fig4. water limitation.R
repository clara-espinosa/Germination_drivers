...
finalgerm %>%
  ...
  geom_segment (aes(x= 1,xend =2,  y = 0.64, yend= 0.64), color = "black", linewidth = 1, show.legend = F)+
  annotate ("text", x= 1.5, y= 0.65, label = "***", size= 4)+
...
finalgerm %>%
  ...
  geom_errorbar(aes(x= community, y=mean, ymin=mean-se, ymax=mean+se), color = "black", linewidth=0.5, width = 0.4)+
  labs (y="Decreased germinationn (%)", subtitle= "b) Decreased germination \n     by Habitat", x= "Habitat")+ #
  theme_classic (base_size = 9) + #theme_minimal for all species for mean treatment
...
### C) germination reduction ordered by germination #####
finalgerm %>%
  merge(read.csv("data/species.csv"), by = c("species", "code"))%>%
  ...
  mutate(germ_reduction=(1-(C_alternate_WP/A_alternate_light))*100)%>%
  mutate_all(~replace(., is.nan(.), 0))%>% # generate Nan because 0 germ pro
  mutate(species= str_replace(species, "Minuartia arctica", "Minuartia sp"))%>%
  convert_as_factor(species, community, petri, community)%>%
  mutate(community= fct_relevel(community, "Mediterranean", "Temperate" ))%>%
...
### D) Control + water limitation germination #####
finalgerm %>%
  merge(read.csv("data/species.csv"), by = c("species", "code"))%>%
  mutate(species = gsub("Helianthemum urrielense", "Helianthemum urrielense*", species))%>%
  filter(treatment== "C_alternate_WP"| treatment == "A_alternate_light")%>%
  ...
  mutate(species = ifelse(species=="Thymus praecox" & community == "Temperate \n Alpine", "Thymus praecox ", as.character(species)))%>%
  group_by(species, community)%>%
  get_summary_stats (C_alternate_WP,A_alternate_light)%>%
  mutate(variable= fct_recode(variable,"Control" ="A_alternate_light", 
                               "Water limitation"="C_alternate_WP" ))%>%
  mutate(variable= fct_relevel(variable, "Control", "Water limitation" ))%>%
  dplyr::select(community, species, variable, mean, se)%>%
  ggplot(aes(x= reorder(species, mean), y= mean,fill = community)) +
  geom_bar (stat = "identity", color = "black") +
  geom_vline(xintercept = 40.5, linetype = "dashed", color = "red", linewidth = 1) +
  facet_grid(~variable)+
  ...
  coord_flip() + 
  labs (subtitle = "c) Germination by Species", y = "Germination proportion")+
  theme_classic (base_size = 9) +
  theme (legend.position = "none",
         axis.title.y= element_blank(),
         axis.text.x= element_text(),
         axis.text.y = element_text(face= "italic"))-> fig3d;fig3d
...
((fig3a / fig3b) | fig3d)+
  plot_layout(widths = c(0.5, 1))-> fig3v2;fig3v2
...
ggsave(filename = "Fig4.png", plot =fig3v2 , path = "results/figures", 
       device = "png", dpi = 600, width = 175, height = 140, units = "mm")
