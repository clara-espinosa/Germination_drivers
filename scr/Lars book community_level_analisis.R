...
# weighted dissimilarity matrix by groups of traits 
...
as.matrix(plant.traits.weighted.dist.M)
...
## 5.6. Calculation of MDP (main pairwise dissimilarity) and RAo with melodic function ####
...
# 5.6.2 Plant traits weighted and not weighted
plot_x_sp_M <- as.matrix(plot_x_sp_M)
...
## 5.6 Calculation of MDP (main pairwise dissimilarity) and RAo with picante package (not working)####
...
plot_x_sp_M <- data.frame(plot_x_sp_M)
Rao.dvc.M <- divc(sp_x_plot_M, germ.traits.dist.M)
...
data.frame(important.indices.M)%>%
  ...
  merge(plot_x_env_M2, by = "plot")%>%
  merge(read.csv("data/spatial-survey-header-Med.csv"), by = c("plot", "elevation"))%>%
  dplyr:: select(site, plot, FDind, value, elevation, FDD, GDD, Snw)%>%
  mutate(FDind = factor(FDind))%>%
  rename(Snow=Snw)%>%
  gather(micro_variable, value_micro,  elevation:Snow)-> FDind_M


  ggplot(FDind_M)+ # GDD  FDD Snw  elevation
  geom_point(aes(y=value, x= value_micro, fill = site), color= "black",shape = 21, size =3)+
  scale_fill_manual( values = c("limegreen","deeppink4","darkorange1", "dodgerblue4"))+
  geom_smooth (aes(y=value, x= value_micro),method = "lm", se=T, color = "black", inherit.aes=F)+
  facet_grid (FDind~micro_variable, scales = "free")+
...
ggsave(plot, file = "CWM Med trait vs micro.png", 
       path = "results/preliminar graphs", scale = 1,dpi = 600) 
...
plant.traits.weighted.dist.T<-gawdis(sp_x_trait_T[,1:5], w.type = "optimized", opti.maxiter = 200, 
                                     groups.weight=T, groups = c(1,2, 3, 3, 3))#
as.matrix(plant.traits.weighted.dist.T)
...
## 5.6 Calculation of MDP (main pairwise dissimilarity) and RAo with picante package ####
