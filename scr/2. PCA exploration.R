...
ggplot(pcaInds_bioclim_M, aes(x = Dim.1, y = Dim.2)) +
  ...
  labs(title= "Bioclimatic indices", subtitle= "Mediterranean community")+
  theme(text = element_text(family = "sans"),
        plot.title = element_text (size= 16),
        plot.subtitle = element_text (size=14),
        legend.position = "bottom", 
        legend.title = element_blank(),
        legend.text = element_text(color = "black", margin = margin(l= 0,r=0, unit = "pt")), 
        legend.key.spacing.x=unit(0.01, "pt"),
  ...
  scale_x_continuous(name = paste("Axis 1 (", round(pca_bioclim_M$eig[1, 2], 0),
                                  "% variance explained)", sep = ""),limits = c(-5, 4.5) ) + # 
...
ggplot(pcaInds_bioclim_T, aes(x = Dim.1, y = Dim.2)) +
  ...
  ggthemes::theme_tufte(base_size=12) + 
  labs(subtitle= "Temperate community")+
  theme(text = element_text(family = "sans"),
        plot.title = element_text (size= 16),
        plot.subtitle = element_text (size=14),
        legend.position = "bottom", 
...
# combine graph
ggarrange(Med_bioclim,Tem_bioclim,  ncol= 2, common.legend = F)
Med_bioclim + Tem_bioclim-> bioclim;bioclim

ggsave(filename = "Figure 1B.png", plot =bioclim , path = "results/Figures", 
       device = "png", dpi = 600)
...
pca_traits_med$var$coord[, 1:2] %>%
  ...
  mutate(Variable = fct_recode(Variable,"Darkness"= "odds_B_dark","Water stress" = "odds_C_WP",
                               "Constant Temp"= "odds_D_constant","Seed mass"="seed_mass",
                               "Plant height"= "plant_height" , "Leaf area"= "leaf_area",
                               "LDMC" = "LDMC", "SLA" = "SLA", ))-> pcaVars_traits_med
...
ggplot(pcaInds_traits_med, aes(x = Dim.1, y = Dim.2)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_vline(xintercept = 0, linetype = "dashed") +
  geom_segment(data = pcaVars_traits_med, aes(x = 0, y = 0, xend = 3*Dim.1, yend = 3*Dim.2), linewidth = 1, arrow = arrow(length = unit(0.3,"cm"))) +
  #geom_point(aes(fill = family), color = "black", show.legend = T, size = 4, shape = 21) + # family
  geom_label_repel(data = pcaVars_traits_med, aes(x = 3*Dim.1, y = 3*Dim.2, label = Variable),  show.legend = F, size = 5, alpha = 0.9, point.padding = NA) +
  ...
  #geom_label_repel(data = pcaVars_traits_med, aes(x = 3*Dim.1, y = 3*Dim.2, label = Variable),  show.legend = FALSE, size = 5, segment.size= 1,
                   #point.padding = 0.2, nudge_x = .15, nudge_y = .5,segment.curvature = -1e-20, segment.linetype = 1, segment.color = "red", arrow = arrow(length = unit(0.015, "npc")))+
  #geom_text_repel (data = pcaInds_traits_med, aes (x = Dim.1, y = Dim.2, label = species), show.legend = F, size =3) +
  ggthemes::theme_tufte(base_size=12) + 
  labs(title= "Traits PCA exploration", subtitle = "Mediterranean community")+
  theme(text = element_text(family = "sans"),
        plot.title = element_text (size= 16),
        plot.subtitle = element_text (size=14),
  ...
  scale_x_continuous(name = paste("Axis 1 (", round(pca_traits_med$eig[1, 2], 0),
                                  "% variance explained)", sep = ""), limits = c(-4, 3.5)) + 
  scale_y_continuous(name = paste("Axis 2 (", round(pca_traits_med$eig[2, 2], 0), 
                                  "% variance explained)", sep = ""), limits = c(-2.5, 3))-> germ.trait.pca.M;germ.trait.pca.M

# geom_text_repel()# Temperate
...
pca_traits_tem$var$coord[, 1:2] %>%
  ...
  mutate(Variable = fct_recode(Variable,"Darkness"= "odds_B_dark","Water stress" = "odds_C_WP",
                               "Constant Temp"= "odds_D_constant","Seed mass"="seed_mass",
                               "Plant height"= "plant_height" , "Leaf area"= "leaf_area",
                               "LDMC" = "LDMC", "SLA" = "SLA", ))-> pcaVars_traits_tem
...
ggplot(pcaInds_traits_tem, aes(x = Dim.1, y = Dim.2)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_vline(xintercept = 0, linetype = "dashed") +
  geom_segment(data = pcaVars_traits_tem, aes(x = 0, y = 0, xend = 3*Dim.1, yend = 3*Dim.2), linewidth = 1, arrow = arrow(length = unit(0.3,"cm"))) +
  #geom_point(aes(fill = family), color = "black", show.legend = T, size = 4, shape = 21) + # family
  geom_label_repel(data = pcaVars_traits_tem, aes(x = 3*Dim.1, y = 3*Dim.2, label = Variable),  show.legend = F, size = 5, alpha = 0.9, point.padding = NA) +
  ...
  scale_y_continuous(name = paste("Axis 2 (", round(pca_traits_tem$eig[2, 2], 0), 
                                  "% variance explained)", sep = ""), limits = c(-2.5, 3))-> germ.trait.pca.T; germ.trait.pca.T

# combine graph
ggarrange(germ.trait.pca.M,germ.trait.pca.T,  ncol= 2, common.legend = F)-> germ.trait.pca;germ.trait.pca
germ.trait.pca.M + germ.trait.pca.T-> germ.trait.pca;germ.trait.pca
ggsave(filename = "Figure 1C.png", plot =germ.trait.pca , path = "results/Figures", 
       device = "png", dpi = 600)

# combine the fours graphs
(Med_bioclim + Tem_bioclim)/(germ.trait.pca.M + germ.trait.pca.T)-> fig1bc;fig1bc
